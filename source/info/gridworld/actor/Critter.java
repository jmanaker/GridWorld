...
{
  //For runner
  public static final boolean[] DRJAVA = new boolean[]
          {
      //Assign this in a runner
              true
          };
  ...
  public Critter()
  {
    this(Rock.class, Critter.class);
    IMMUNE = false;
  }
  public Critter(Class<?>... types)
  {
    this.types = new ArrayList<>(Arrays.asList(types));
    if(!this.types.contains(Critter.class))
    {
        this.types.add(Critter.class);
    }
    IMMUNE = true;
  }
  /**
  ...
  public void processActors(ArrayList<Actor> actors)
  {
    for (Actor a : actors)
    {
      //This fails only in DrJava; DrJava has trouble with wrappers and C# boxing
      if(!DRJAVA[0])
      {
        if(!types.contains(a.getClass()))
        {
          a.removeSelfFromGrid();
        }
      }
      //Thus, set Critter.DRJAVA to true to use this
      else
      {
        ...
        if(IMMUNE)
        {
          try
          {
            //The myth, the legend, the...possibly loaded class?
            //Thus, no instanceof or class literal
            if(Class.forName("Dragon").isAssignableFrom(a.getClass()))
            {
              continue;
            }
          }
